...
def create_dh_key():
    ...
    privateKey = random.randint(2, prime - 2)
    publicKey = pow(2,privateKey,prime)
    return (publicKey,privateKey)

def validKey(shared_key1, shared_key2):
    if(shared_key1 == shared_key2):
        return 1
    else:
        return 0

def calculate_secret(their_public, my_private):
...
def calculate_dh_secret(their_public, my_private):
    # Calculate the shared secret
    shared_secret = calculate_secret(their_public, my_private)
    ...
    shared_hash = shared_hash.encode("ascii")
    return shared_hash
